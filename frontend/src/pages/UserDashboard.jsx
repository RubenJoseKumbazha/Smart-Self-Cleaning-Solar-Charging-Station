...
export default function UserDashboard() {
  ...
  const { data: benches, loading, error, updatedAt } = useFetch(fetchBenches, [], 300000);
  ...
  return (
    <div className="space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Solar Bench Monitoring</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">View status and monitor bench performance across the network.</p>
        </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-700 dark:bg-slate-900">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold">Monitored Benches</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Click any bench to view details and control options.</p>
          </div>
          <Link
            to="/analytics"
        ...
        {!loading && benches?.length === 0 && (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
